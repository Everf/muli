...
void LFGQueue::RemoveFromCompatibles(ObjectGuid guid)
{
    ...
    //sLog.outDebug("LFGQueue::RemoveFromCompatibles: Removing [" UI64FMTD "]", guid);
...
uint8 LFGQueue::FindGroups()
{
    ...
    while (!newToQueueStore.empty())
    {
        ...
        //sLog.outDebug("LFGQueue::FindGroups: checking [" UI64FMTD "] newToQueue(%u), currentQueue(%u)", frontguid, uint32(newToQueueStore.size()), uint32(currentQueueStore.size()));
        firstNew.clear();
...
LfgCompatibility LFGQueue::FindNewGroups(GuidList& check, GuidList& all)
{
    ...
    //sLog.outDebug("LFGQueue::FindNewGroup: (%s): %s - all(%s)", strGuids.c_str(), GetCompatibleString(compatibles), ConcatenateGuids(all).c_str());
    ...
    {
        //sLog.outDebug("LFGQueue::FindNewGroup: (%s) compatibles (cached) changed from bad states to match", strGuids.c_str());
...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s): Size wrong - Not compatibles", strGuids.c_str());
    ...
    if (check.size() > 2)
    {
        ...
        {
            //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) child %s not compatibles", strGuids.c_str(), ConcatenateGuids(check).c_str());
    ...
    for (GuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 2 && numPlayers <= MAX_GROUP_SIZE; ++it)
    {
        ...
        {
            ObjectGuid guid = ObjectGuid(itQueue->first);
            if (guid.IsGroup())
                proposalGroups[it2->first] = itQueue->first;
            else
                proposalGroups[it2->first] = ObjectGuid();
        }
    ...
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) sigle group. Compatibles", strGuids.c_str());
    ...
    if (numLfgGroups > 1)
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) More than one Lfggroup (%u)", strGuids.c_str(), numLfgGroups);
    ...
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) Too much players (%u)", strGuids.c_str(), numPlayers);
    ...
    if (check.size() > 1)
    {
        ...
        {
            //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) not compatible, %u players are ignoring each other", strGuids.c_str(), playersize);
        ...
        if (!LFGMgr::CheckGroupRoles(proposalRoles))
        {
            ...
            //    o << ", " << it->first.GetCounter() << ": " << sLFGMgr.GetRolesString(it->second);

            //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) Roles not compatible%s", strGuids.c_str(), o.str().c_str());
        ...
        {
            //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) No compatible dungeons%s", strGuids.c_str(), o.str().c_str());
    ...
    if (numPlayers != MAX_GROUP_SIZE)
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) Compatibles but not enough players(%u)", strGuids.c_str(), numPlayers);
        LfgCompatibilityData data(LFG_COMPATIBLES_WITH_LESS_PLAYERS);
    ...
    {
        //sLog.outDebug("LFGQueue::CheckCompatibility: (%s) Group MATCH but can't create proposal!", strGuids.c_str());
    ...
    proposal.leader = ObjectGuid();
    LfgDungeonSet::iterator itr = proposalDungeons.begin();
    std::advance(itr, urand(0, proposalDungeons.size() - 1));
    proposal.dungeonId = *itr;
...
{
    //sLog.outDebug("LFGQueue::FindBestCompatibleInQueue: " UI64FMTD, itrQueue->first);
    std::ostringstream o;
    o << itrQueue->first.GetCounter();
    std::string sguid = o.str();
...
void LFGQueue::UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, std::string const& key, LfgRolesMap const& roles)
{
    ...
    //sLog.outDebug("LFGQueue::UpdateBestCompatibleInQueue: Changed (%s) to (%s) as best compatible group for " UI64FMTD,
